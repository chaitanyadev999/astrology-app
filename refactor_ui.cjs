...
// Target JSX block
const startMatch = `{step==='result' && result && (`;
const sIdx = code.indexOf(startMatch);

if (sIdx > -1) {
  // Let's replace from startMatch up to `      )}\n    </div>`
  
  const endReturnStr = `      )}\n    </div>\n  );\n}`;
  const endReturn = code.lastIndexOf(endReturnStr);
  ...
  if (endReturn > -1) {
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    <div className="bg-slate-900/50 border border-slate-700 rounded-2xl p-4">
                      ...
                      <div className="grid grid-cols-2 gap-3 text-xs">
                        ...
                        <div><span className="text-slate-400 block mb-1">జన్మ లగ్నం:</span> <span className="font-bold text-purple-400">{RASHIS[uRes.userObj?.lagnam || 0]} ({PLANETS[uRes.lagP]})</span></div>
                        <div><span className="text-slate-400 block mb-1">జన్మ రాశి:</span> <span className="font-bold text-purple-400">{RASHIS[uRes.userObj?.rashi || 0]} ({PLANETS[uRes.rashiP]})</span></div>
                      </div>
                    </div>
    ...
    console.log('Could not find end Return precisely');
  }
} else {
